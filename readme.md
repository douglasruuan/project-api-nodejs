...
**API RESTful:** Quando você separa o **front-end** do **back-end**, então você tem o **back-end** que você manipula as informações do Banco de Dados receber requisições e enviar respostas.
...
**Quais Benefícios de usarmos uma API RESTful?**

  

  

Uma **API RESTful** oferece para gente mais flexibilidade no nosso **front-end.**
...
- -d (qual imagem queremos usar que e o mongo).
  ...
  exemplo: `docker run -name -p27017:27017 -d`
...
const routes = express.Router();
	/**
	*Variavel para as rotas do express.
	*/
...
const mongoose = require("mongoose");
	...
	*/
	...
	/**
	*Importar nosso model de product
	*/
...
module.exports = {
	...
	async store(req, res) {
		const product = await Product.create(_req_.body);
			/**
			*Fazendo a requisão do corpo (body)
			*/
		return _res_.json(product);
			/**
			*Retornando em formato de json o product.
			*/
	},
...
module.exports = {
	async index(req, res) {
		...
		const products = await Product.paginate({}, {
				/**
				*Dentro de paginate iremos passar um objeto vazio.
				*Se quisermos passar algum WHERE depois.
				*/
			page,
				/**
				*Pagina Atual
			...
			limit: 10 
				/**
				*Limite de dados que vamos retornar por pagina
				*/
});
```
